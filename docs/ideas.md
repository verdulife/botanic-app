...
4. 🟡 **Emails en spam** — auditoría inicial completada (Hito 1 ✅) y fixes aplicados (Hito 2 ✅: FROM `hola@`, replyTo gmail, List-Unsubscribe headers, footer físico-legal, header image 600px). Pendiente: re-test Outlook con código nuevo (Hito 3) y decidir si se activan Open/Click tracking en Resend. Detalle completo + plan en [`docs/email-deliverability.md`](docs/email-deliverability.md).
...
19. **Revisar `waitlist_count` para paridad con la realidad** — la view `public.waitlist_count` (`SELECT count(*) FROM waitlist`) devuelve el mismo número que `COUNT(*)` directo sobre la tabla (34 vs 34, comprobado). Garantías actuales: `email` UNIQUE evita duplicados; RLS solo permite INSERT a anon; no hay soft-delete. **Riesgos latentes a vigilar**: (a) condiciones de carrera durante picos de registro (`count(*)` no ve transacciones en vuelo,pequeña desviación transitoria); (b) deletes manuales fuera de RLS (service_role puede borrar filas sin que la view lo refleje automáticamente — la view ya está sincronizada, pero no hay validación continua); (c) migraciones futuras que alteren la tabla sin actualizar la view; (d) la anon key tiene grants `DELETE/UPDATE/TRUNCATE` sobre la view (no solo SELECT) — superficie de ataque innecesaria. **Mejoras posibles**: trigger que mantenga una tabla contador denormalizada (`waitlist_counter` con 1 fila) y la actualice en `AFTER INSERT/DELETE`; validación periódica en el cron de keep-alive (compara `waitlist_count.total` con `COUNT(*)` directo y alerta si difieren); reducir grants de anon a solo `SELECT` sobre la view.
