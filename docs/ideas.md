...
18. ✅ **Cron keep-alive de Supabase Free** — implementado en `.github/workflows/keep-alive-supabase.yml` (cron diario `0 6 * * *` UTC ≈ 7-8 AM España + `workflow_dispatch` para test manual). Ejecuta `psql "$SUPABASE_DB_URL" -c "SELECT 1;" --set ON_ERROR_STOP=1` contra el Transaction pooler (`aws-0-eu-central-1.pooler.supabase.com:6543`). Secret `SUPABASE_DB_URL` en GitHub Actions con la URI completa. Notificación nativa de GH configurada (`Settings → Notifications → Workflows → Failed only`) como canal de alerta. Detalle histórico del problema: Supabase Free pausa el proyecto tras **7 días sin queries** al PostgreSQL, dejándolo inaccesible hasta reactivarlo a mano. **Cuando pasemos a Supabase Pro**, el cron deja de ser necesario para evitar la pausa (mantener solo como health check o desinstalar).
