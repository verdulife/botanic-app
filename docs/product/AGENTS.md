# Product — Contexto del módulo (negocio, no técnico)

Documentación de contexto de negocio que respalda decisiones de producto. No es código — cárgalo para tareas de estrategia, copy, difusión o pricing, no para tareas de implementación. Fuente canónica de producto sigue siendo [PRODUCT.md](../../PRODUCT.md) (raíz).

## Documentación del módulo

| Doc | Contenido | Ubicación |
|---|---|---|
| mercado | Datos de mercado y análisis de competencia (Wallapop, Vinted, Milanuncios) | [mercado.md](mercado.md) |
| monetizacion | Modelo de negocio y planes (Gratis / Pro / Profesionales) | [monetizacion.md](monetizacion.md) |
| difusion | Plan de difusión pre y post lanzamiento, canales y cadencia | [difusion.md](difusion.md) |

---

> **Actualización**: ver [AGENTS.md § Cómo mantener los docs](../../AGENTS.md#cómo-mantener-los-docs).
