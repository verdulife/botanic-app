# Concepto

## Elevator Pitch

**Botanic** es el marketplace de plantas entre particulares. Una plataforma donde cualquiera puede publicar semillas, esquejes, brotes, plantas y tiestos para venderlos a gente cerca de su zona. Como Wallapop, pero solo de plantas.

## Problema

- Hoy es más fácil vender una chaqueta usada que un esqueje de Monstera.
- Las plataformas generalistas (Wallapop, Vinted, Milanuncios) tienen las plantas como subcategoría perdida entre muebles, ropa y electrónica.
- No hay filtros específicos para especies vegetales, no hay lenguaje común, no hay comunidad.
- Los intercambios ocurren en grupos de Facebook, foros o WhatsApp, de forma desorganizada.

## Solución

1. **Especialización** — Categorías pensadas para el mundo vegetal
2. **Cercanía** — Búsqueda por ubicación para quedar en mano
3. **Comunidad** — Perfiles, valoraciones, reputación entre Plant Lovers
4. **Sencillez** — Publicar un anuncio lleva menos de un minuto

## Público objetivo

- **Plant lovers** con esquejes que les sobran
- **Aficionados a la jardinería** con semillas o excedente
- **Personas que se mudan** y necesitan dar salida a macetas y plantas
- **Coleccionistas** de variedades específicas
- **Principiantes** que buscan plantas baratas cerca

## Diferenciación

| Dimensión | Botanic | Wallapop | Vinted | Palmstreet | Plant Position |
|---|---|---|---|---|---|
| Enfoque | Solo plantas | Generalista | Ropa | Plantas + lifestyle | Viveros (B2B) |
| Modelo | P2P | P2P | P2P | Live shopping | Profesional |
| España | ✅ | ✅ | ✅ | ❌ | ✅ |
| Sin comisiones | ✅ (MVP) | Parcial | Parcial | Parcial | ❌ |
