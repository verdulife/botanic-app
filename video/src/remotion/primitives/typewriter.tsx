...
import { EASING_ENTER } from "../lib/timing";
...
export type TypewriterProps = {
  ...
  /** Substring (of the rendered text) to render with extra weight. */
  highlight?: string;
...
export const Typewriter: React.FC<TypewriterProps> = ({
  ...
  const hiStart =
    highlight && highlight.length > 0
      ? timeline.displayText.indexOf(highlight)
      : -1;
  const hiLen = highlight?.length ?? 0;

  const renderSegments = (str: string, start: number, end: number) => {
    if (hiStart < 0 || start >= end) return str;
    const s = Math.max(start, hiStart);
    const e = Math.min(end, hiStart + hiLen);
    if (s >= e) return str;
    return (
      <>
        {str.slice(0, s - start)}
        <strong style={{ fontWeight: highlightWeight }}>
          {str.slice(s - start, e - start)}
        </strong>
        {str.slice(e - start)}
      </>
    );
  ...
  const typed = (
    <>
      {renderSegments(typedText, 0, visibleChars)}
      {showCursor ? (
        <Cursor
          frame={frame}
  ...
  return (
    <span style={{ ...textStyle, display: "inline-grid" }}>
      <span
        ...
        aria-hidden
      >
        {renderSegments(timeline.displayText, 0, timeline.displayText.length)}
      </span>
