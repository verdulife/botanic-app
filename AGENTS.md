...
- **Redes sociales (carril paralelo)**: implementado a nivel PoC. Motor: **Remotion** en `video/` (carpeta autónoma con package.json propio, fuera de workspaces y de la build de Vercel; browser = Chrome Headless Shell vía `npx remotion browser ensure`; **`remotion.config.ts` apunta el publicDir a `../static`** para que los assets de los guiones (`static/social/<slug>/`) se resuelvan con `staticFile()`). Guion agéntico `script.json` (= inputProps): el agente escribe **datos**, nunca animación. Scripts `scripts/stock.mjs` (Pexels: `search`/`search:video`/`fetch`/`fetch:video`, fotos + clips verticales, atribución no requerida; clave `PEXELS_API_KEY` en `.env.local`, Bun la carga sola — **no usar `loadEnvFile`**, falla en Bun). 3 motion styles `energy`/`cozy`/`minimal`. Composición `BotanicReel` (reel 9:16) + `BotanicSlide` (carrusel 4:5; cada still = frame de escena). Render: `bunx remotion still/render src/index.ts <comp> ... --props=<script.json>`. **PoC validado**: reel mp4 17s (960 KB) y carrusel con foto Pexels de fondo. Publicación manual (Meta Business Suite + TikTok Studio) en batch semanal: reels 3-4x/semana + posts/carruseles 2-3x/semana. **No bloquea el MVP.** Skill oficial `remotion-best-practices` en `.opencode/skills/`. Detalle en [`docs/social-video.md`](docs/social-video.md) y [`docs/social-post.md`](docs/social-post.md). **Catálogo de elementos 9:16**: composición `Catalog` en `video/src/catalog/` (slots etiquetados INTRO/HOOK/TIP/QUOTE/OUTRO/TRANSICIÓN/ENDING, empieza **vacía**). Ritual por elemento: el usuario elige un componente → instalar → restilizar a marca → revisar en Studio → `git commit` + entrada en `video/CATALOG.md` (inventario). `remotion-ui` inicializado (`remotion-ui.json`; añadió `@/*` → `./src/*` al tsconfig; el `baseUrl` que genera depreca en TS 6 → eliminado). **Librerías evaluadas**: **RemotionUI** adoptado (CLI `npx remotion-ui add <c>`, source-you-own); **Onda = `ondajs`** (degueba/onda, CLI `npx ondajs add <c>`, instala en `components/onda/`, theming por CSS vars → re-skinnea a tokens de marca) — ojo: **onda.video (onda-engine) NO es una librería Remotion** (motor GPU independiente), descartado; TikTokTextBox/SwiftClip solo a petición. **Pendientes del carril social**: sesiones de catálogo por elemento (Hito 1 = INTRO), cablear elementos aprobados en `BotanicReel`/`BotanicSlide` (eliminar duplicación: `SlideContent` inline, `Outro`/`ProgressDots` duplicados, `Tip` sin soporte vídeo), re-render + auditoría visual, clip de vídeo en tips, audio Fase 2, batch semanal.
...
1. **Finalizar carril social** — checklists en [`docs/social-video.md`](docs/social-video.md) y [`docs/social-post.md`](docs/social-post.md).
