...
- **Cron keep-alive de Supabase Free** ✅: workflow `.github/workflows/keep-alive-supabase.yml` (cron diario `0 6 * * *` UTC ≈ 7-8 AM España + `workflow_dispatch`). Ejecuta `SELECT 1` contra el Transaction pooler (`aws-0-eu-central-1.pooler.supabase.com:6543`) usando el secret `SUPABASE_DB_URL`. Evita la pausa automática de Supabase Free tras 7 días sin queries. Notificación nativa de GitHub configurada para fallos (`Settings → Notifications → Workflows → Failed only`).
